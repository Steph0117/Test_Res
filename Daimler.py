...
class Daimler(imdb):
    def __init__(self, version, image_set, devkit_path="otherdata"):
        imdb.__init__(self,'otherdata' + image_set)
        ...
        self._data_path = os.path.join("/root/data",self._devkit_path, 'Daimler')
        self._classes = ('__background__', # always index 0
                         'person')
        self._class_to_ind = dict(zip(self.classes, xrange(self.num_classes)))
        annotation_path = os.path.join(self._data_path, "Daimler_annotations.json")
...
#Strategy: get the index from annotation dictionary 
    ...
    def person_class_index(self, image_set_list):
        ...
        for image_name in image_index :
            ...
            boxes = self._annotation[set_num][v_num]["frames"][frame_num].values()
            print boxes
    ...
    def _load_caltech_annotation(self, index):
        ...
        bboxes = self._annotation[set_num][v_num]["frames"][frame_num].values()
        ...
        boxes = np.zeros((num_objs, 4), dtype=np.uint32)#original uint16
